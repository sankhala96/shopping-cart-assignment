...
const confirmPassword = (value1, value2) => (value1.valueOf().trim() === value2.valueOf().trim())
    ? '' : Constants.ErrorConfirmPassword;

export const validate = (checkIf, value, ...otherValues) => {
    switch(checkIf) {
        case Constants.Required: return required(value);
        case Constants.Email: return email(value);
        case Constants.Password: return password(value);
        case Constants.ConfirmPassword: return confirmPassword(value, otherValues[0]);
        default: return Constants.ErrorInvalid;
    }
}